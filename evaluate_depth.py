...
from options import LiteHDGFOptions
import datasets
import networks
import time
...
def profile_once(encoder, decoder, x):
    x_e = x[0, :, :, :].unsqueeze(0)
    x_d, fake_images, real_images = encoder(x_e)
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        assert os.path.isdir(opt.load_weights_folder), \
            "Cannot find a folder at {np.int}".format(opt.load_weights_folder)
        ...
        encoder = networks.LiteHDGF(model=opt.model,
        ...
        with torch.no_grad():
            for data in dataloader:
                ...
                t1 = time_sync()
                features, fake_images, real_images = encoder(input_color)
    ...
    eval_txt_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "eval_txt", "baloss7")
    os.makedirs(eval_txt_dir, exist_ok=True)
    if opt.load_weights_folder:
        model_name = os.path.basename(os.path.normpath(opt.load_weights_folder))
    else:
        model_name = "evaluation"
    eval_file_path = os.path.join(eval_txt_dir, "{}_results.txt".format(model_name))
...
if __name__ == "__main__":
    options = LiteHDGFOptions()
    opt = options.parse()
    
    opt.load_weights_folder = "/data/zhangyawei/Lite-HDGF/Lite-HDGF"
    print(f"Evaluating {opt.load_weights_folder}...")
    evaluate(opt)
