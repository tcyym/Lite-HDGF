...
class GFRefine(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, expan_ratio=6,
        ...
        self.dgif = DGIF(num_features=self.dim//2, act='PReLU', norm=None, filter_size=3)

        self.gamma_xca = nn.Parameter(layer_scale_init_value * torch.ones(self.dim//2),
                                      requires_grad=True) if layer_scale_init_value > 0 else None
        self.xca = XCA(self.dim//2, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
            ...
            for j in range(self.depth[i]):
                if j > self.depth[i] - global_block[i] - 1:
                    if global_block_type[i] == 'LGFI':
                        ...
                        # stage_blocks.append(DGIF(num_features=self.dims[i], act='PReLU', norm=None, filter_size=3))
                    else:
                        raise NotImplementedError
                else:
                    stage_blocks.append(DilatedConv(dim=self.dims[i], k=3, dilation=self.dilation[i][j], drop_path=dp_rates[cur + j],
