...
class CCF(nn.Module):
    """ Cross-Covariance Attention (CCF) operation where the channels are updated using a weighted
...
class DilatedConv(nn.Module):
    """
    A single Dilated Convolution layer in the Consecutive Dilated Convolutions (HDCM) module.
    """
    def __init__(self, dim, k, dilation=1, stride=1, drop_path=0.,
        ...
        self.conv111 = Conv(dim, dim1, kSize=1, stride=1)

        self.ddwconv = HDCM(dim1, dim1, kSize=k, stride=stride, groups=1, d=dilation)
        self.ddwconv1 = HDCM(dim1, dim1, kSize=k, stride=stride, groups=1, d=1)
        self.bn1 = nn.BatchNorm2d(dim1)

        # self.conv112 = Conv(dim1, dim, kSize=1, stride=1)
        self.norm = LayerNorm(dim1, eps=1e-6)
        # self.pwconv1 = nn.Linear(dim, dim1)
        self.act = nn.GELU()
        self.act1 = nn.GELU()
        self.pwconv2 = nn.Linear(dim1, dim)
        # self.gamma = nn.Parameter(layer_scale_init_value * torch.ones(dim),
        #                           requires_grad=True) if layer_scale_init_value > 0 else None

        # self.conv111 = Conv(dim, dim, kSize=1, stride=1)
        #
        # self.ddwconv = HDCM(dim, dim, kSize=k, stride=stride, groups=dim, d=dilation)
        # self.bn1 = nn.BatchNorm2d(dim)
        #
        # self.conv112 = Conv(dim, dim, kSize=1, stride=1)
...
#
#         self.ddwconv = HDCM(dim, dim, kSize=k, stride=stride, groups=dim, d=dilation)
#         self.ddwconv1 = HDCM(dim, dim, kSize=k, stride=stride, groups=1, d=1)
#         self.bn1 = nn.BatchNorm2d(dim)
#
#         # self.conv112 = Conv(dim1, dim, kSize=1, stride=1)
#         self.norm = LayerNorm(dim, eps=1e-6)
...
class GFRefine(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, expan_ratio=6,
        ...
        self.pos_embd = None
        if use_pos_emb:
            self.pos_embd = PositionalEncodingFourier(dim=self.dim//2)
            
            
        self.down = nn.Linear(dim, dim // 2, bias=qkv_bias)

        self.norm_xca = LayerNorm(self.dim//2, eps=1e-6)
        self.dgif = AFGF(num_features=self.dim//2, act='PReLU', norm=None, filter_size=3)

        self.gamma_xca = nn.Parameter(layer_scale_init_value * torch.ones(self.dim//2),
                                      requires_grad=True) if layer_scale_init_value > 0 else None
        self.xca = CCF(self.dim//2, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)

        self.norm = LayerNorm(self.dim//2, eps=1e-6)
        self.pwconv1 = nn.Linear(self.dim // 2, self.dim)
        # self.act = nn.GELU()
...
class LGFI(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, expan_ratio=6,
        ...
        self.gamma_xca = nn.Parameter(layer_scale_init_value * torch.ones(self.dim),
                                      requires_grad=True) if layer_scale_init_value > 0 else None
        self.xca = CCF(self.dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
...
# class LiteHDGF(nn.Module):
#     """
#     Lite-Mono
#     """
#     def __init__(self, in_chans=3, model='lite-HDGF', height=192, width=640,
#                  global_block=[1, 1, 1], global_block_type=['GFRefine+', 'GFRefine+', 'GFRefine+'],
...
#             elif height == 320 and width == 1024:
#                 self.dilation = [[1, 2, 5], [1, 2, 5], [1, 2, 5]]
#
#         elif model == 'lite-HDGF-tiny':
...
#                     if global_block_type[i] == 'GFRefine+':
#                         stage_blocks.append(GFRefine(dim=self.dims[i], drop_path=dp_rates[cur + j],
...
#                 x = self.stages[i][s](x)
#             x = self.stages[i][-1](x, x_down[i+1])
#             tmp_x.append(x)
#
#             features.append(x)
#
...
class LiteHDGF(nn.Module):
    ...
    def __init__(self, in_chans=3, model='lite-HDGF', height=192, width=640,
        ...
        self.downsample_layers.append(stem1)
        self.fake_images = nn.ModuleList()
        for i in range(0, 3):
            self.fake_images.append(Conv(self.dims[i], 3, kSize=1, stride=1))
            ...
            for j in range(self.depth[i]):
                if j > self.depth[i] - global_block[i] - 1:
                    if global_block_type[i] == 'GFRefine+':
                        ...
                        # stage_blocks.append(AFGF(num_features=self.dims[i], act='PReLU', norm=None, filter_size=3))
                    else:
                        raise NotImplementedError
                else:
                    stage_blocks.append(DilatedConv(dim=self.dims[i], k=3, dilation=self.dilation[i][j], drop_path=dp_rates[cur + j],
    ...
    def forward_features(self, x):
        ...
        for s in range(len(self.stages[0])-1):
            x = self.stages[0][s](x)
        x = self.stages[0][-1](x, x_down[1])
        # x = self.stages[0][-1](x, x_down[0:2])
        fake_image = self.fake_images[0](x)
        ...
        for i in range(1, 3):
            ...
            for s in range(len(self.stages[i]) - 1):
                x = self.stages[i][s](x)
            x = self.stages[i][-1](x, x_down[i+1])
            # x = self.stages[i][-1](x, x_down[i : i + 2])
            fake_image = self.fake_images[i](x)
        ...
        return features, fake_images, x_down[1:4]

    def forward(self, x):
        x, fake_images, x_down = self.forward_features(x)

        return x, fake_images, x_down
