...
class FuseBlock(nn.Module):
    def __init__(self, num_feature, act, norm, kernel_size, scale=2):
        ...
        # self.xca = encod.CCF(dim=kernel_size ** 2, num_heads=9, qkv_bias=True, attn_drop=0, proj_drop=0)
...
class AFGF(nn.Module):
    ...
    def forward(self, lr, rgb):
        ...
        scharr_x = torch.FloatTensor(scharr_x).expand(c, 1, 3, 3)
        scharr_y = torch.FloatTensor(scharr_y).expand(c, 1, 3, 3)
        scharr_x = scharr_x.type_as(rgb)
        scharr_y = scharr_y.type_as(rgb)
        weight_x = torch.nn.Parameter(data=scharr_x, requires_grad=False)
        weight_y = torch.nn.Parameter(data=scharr_y, requires_grad=False)
