...
import networks.switchable_norm as sn
...
class Head(nn.Module):
    def __init__(self, num_features, act, norm, guide_channels=3, in_channels=3):
        super(Head, self).__init__()

        self.depth_head = nn.Sequential(
            # ConvBNReLU2D(in_channels=in_channels*2, out_channels=num_features, kernel_size=3, padding=1, act=act, norm=norm),
            ConvBNReLU2D(in_channels=in_channels, out_channels=num_features, kernel_size=1)
        ...
        self.guide_head = nn.Sequential(
            # ConvBNReLU2D(in_channels=guide_channels, out_channels=num_features, kernel_size=3, padding=1, act=act, norm=norm),
            ConvBNReLU2D(in_channels=guide_channels, out_channels=num_features, kernel_size=1)
            # ConvBNReLU2D(in_channels=num_features * expand_ratio, out_channels=num_features, kernel_size=1, act=act)
        )
